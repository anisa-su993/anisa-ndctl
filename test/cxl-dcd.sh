...
# Try to resize a DC dax device via reconfigure -s; must fail because
...
test_remove_extent_under_dax_device()
{
	...
	# DC dax seed starts at size 0; --uuid "0" is the only sizing path.
...
{
	echo ""
	echo "Test: Remove extents under DC dax device"
	echo ""
...
test_create_dax_dev_spanning_two_extents()
{
	...
	echo "Checking if dev dax is spanning DC extents"
...
test_untagged_extents_not_grouped()
{
	echo ""
	echo "Test: untagged extents in one More-chain are independent, not one group"
	echo ""

	region=$(create_dcd_region ${mem} ${decoder})
	check_region ${region} ${dra_size}

	# Two untagged extents delivered in a single More-chain.  A null tag
	# carries no allocation identity, so each is an independent extent and
	# must NOT be coalesced into one tag group.  (Regression guard:
	# extract_tag_group used to uuid_equal-match every untagged extent into
	# one group.)  Leave them unclaimed so a release is immediate, not
	# deferred by a dax-device hold.
	inject_extent ${device} $base_ext_dpa $base_ext_length "" 1
	check_extent_cnt ${region} 0   # held until More=0 closes the chain
	inject_extent ${device} $pre_ext_dpa $pre_ext_length ""
	check_extent_cnt ${region} 2

	# Release ONLY the base extent.  With per-extent groups the pre extent
	# survives (count 1).  If the two were wrongly grouped, the release
	# would tear down the whole containing tag group atomically (count 0) --
	# this is the observable bit that distinguishes the fix.
	remove_extent ${device} $base_ext_dpa $base_ext_length ""
	check_extent_cnt ${region} 1

	# The surviving untagged extent is independently removable.
	remove_extent ${device} $pre_ext_dpa $pre_ext_length ""
	check_extent_cnt ${region} 0
...
{
	echo ""
	echo "Test: Fail DC dax dev creation without space"
	echo ""
	region=$(create_dcd_region ${mem} ${decoder})
	check_region ${region} ${dra_size}
	inject_extent ${device} $pre_ext_dpa $pre_ext_length ""
	...
	# Plain create-device on DC dax: size grow is -EOPNOTSUPP
	...
	echo ""
	echo "Test: Resize of DC dax device to non-zero is rejected"
	echo ""
	...
	# |         |                   | daxX.2            |               |
	echo ""
	echo "Test: Destroy (size=0) of DC dax device releases the resource"
	echo ""
	destroy_dax_dev ${dax_dev}
	check_not_dax_dev ${region} ${dax_dev}
...
# --uuid <unknown> on a DC dax region must return -ENOENT before any
